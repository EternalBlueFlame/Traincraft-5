        ...
        if (!getWorld().isRemote) {
            if (updateTicks % 20 == 0) {
                if (!this.getWorld().isAirBlock(this.xCoord, this.yCoord + 1, this.zCoord)) {
                    ...
                    if (block != null) {
                        EntityItem entityitem = new EntityItem(getWorld(), this.xCoord, this.yCoord + 1, this.zCoord, new ItemStack(Item.getItemFromBlock(BlockIDs.MILWSwitchStand.block), 1));
                        float f3 = 0.05F;
