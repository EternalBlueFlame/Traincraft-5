...
public class TrainsSignal {
    ...
    public static int isSignal(double motionX, double motionZ, int i, int j, int k, World world) {
        ...
        //@SuppressWarnings("unused") TileSignal te = (TileSignal) world.getTileEntity(i, j, k);

        if ((motionX > 0) && motionZ == 0) {
            for (int t = 0; t < 12; t++) {
                if ((world.getBlock(i + t, j, k + l) == BlockIDs.signal.block) || (world.getBlock(i + t, j, k - l) == BlockIDs.signal.block)) {// k = sides, j= heigh, i front
                    action = 1;
                    if (t == 2) {
                        action = 2;
                    }
            ...
            // k - = front
            for (int t = 0; t < 12; t++) {
                if ((world.getBlock(i + l, j, k + t) == BlockIDs.signal.block) || (world.getBlock(i - l, j, k + t) == BlockIDs.signal.block)) {// k = sides, j= heigh, i front
                    action = 1;
                    if (t == 2) {
                        action = 2;
                    }
