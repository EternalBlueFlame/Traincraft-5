                    ...
                    if (obj instanceof Locomotive) {
                        ...
                        if (daTrain.mtcOverridePressed) { return;}
                         if (daTrain.mtcStatus == 0  && hadSentMTCPacket == false) {
                             daTrain.mtcStatus = 1;
                            Traincraft.mscChannel.sendToAllAround(new PacketMTC(daTrain.getEntityId(),   daTrain.mtcStatus, 1)  , new NetworkRegistry.TargetPoint(this.world.provider.dimensionId, daTrain.posX, daTrain.posY, daTrain.posZ, 150.0D));
                        ...
                        if (!world.isRemote && hadSentPacket == false) {

                            trainNumber = daTrain.getEntityId();
							hadSentPacket = true;
                            Traincraft.itsChannel.sendToAllAround(new PacketSetSpeed(setSpeed, getPos(), daTrain.getEntityId()) , new NetworkRegistry.TargetPoint(this.world.provider.dimensionId, daTrain.posX, daTrain.posY, daTrain.posZ, 150.0D));
                                ...
                                Traincraft.itnsChannel.sendToAllAround(new PacketNextSpeed(this.nextSpeedLimit, getPos(), this.xFromSpeedChange, this.yFromSpeedChange,this.zFromSpeedChange, daTrain.getEntityId()) , new NetworkRegistry.TargetPoint(this.world.provider.dimensionId, daTrain.posX, daTrain.posY, daTrain.posZ, 150.0D));
